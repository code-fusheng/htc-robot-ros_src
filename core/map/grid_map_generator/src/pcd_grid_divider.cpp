...
bool pcd_grid_divider::do_process() {
...
void pcd_grid_divider::cb_grid_map_create_task(const automotive_msgs::GridMapCreateConfigConstPtr &msg) {
  
  this->setInFolder(msg->source_dir); // or input file
  this->setOutFolder(msg->dst_dir);
  this->setGridSize(msg->grid_size);
  this->setVoxelSize(msg->voxel_size);

  automotive_msgs::GridMapCreateStatus res_msg;
  res_msg.task_status = res_msg.TASK_ON;
  this->pub_grid_task_status.publish(res_msg);

  bool res = this->do_process();
  
  res_msg.task_status = (res == true) ? res_msg.TASK_FINISHED : res_msg.TASK_ERROR;
  this->pub_grid_task_status.publish(res_msg);
}
