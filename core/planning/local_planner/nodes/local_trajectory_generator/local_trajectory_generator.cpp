...
void LocalTrajectoryGenerator::initROS()
{
    ...
    sub_detectedObjects = nh.subscribe("/detection/objs", 1, &LocalTrajectoryGenerator::getDetectedObjects_cb, this);
    ...
    pub_critical_safe_area = nh.advertise<geometry_msgs::PolygonStamped>("critical_safe_area", 1);
    ...
    nh_private.param<std::string>("lidar_frame", lidar_frame, "");
...
void LocalTrajectoryGenerator::set_detectObj(UtilityNS::DetectedObject& out_obj, const smartcar_msgs::DetectedObjectArrayConstPtr& msg, const double& i)
{
    ...
    velodyne_map_tf = FindTransform("map", lidar_frame);
...
void LocalTrajectoryGenerator::run()
{
    ...
    double pre_index = plan_params.rollOutNumber / 2;
    double pre_pre_index = plan_params.rollOutNumber / 2;
    while (ros::ok()) {
        ros::spinOnce();
        UtilityNS::TrajectoryCost best_trajectory;

        // ROS_INFO("currentPose_flag: %d, centralPathSection.size: %d, velodyne_map_tf_ok: %d", currentPose_flag, centralPathSection.size(), velodyne_map_tf_ok);
        if (currentPose_flag && centralPathSection.size() > 0 && velodyne_map_tf_ok) {
            ...
            // 临时方案: 快到终点时，不执行规划
            if (std::sqrt(std::pow(current_pose.pos.x-centralPathSection.back().pos.x, 2) + std::pow(current_pose.pos.y-centralPathSection.back().pos.y, 2)) < 3) {
                best_index = int(plan_params.rollOutNumber / 2);
            }
    ...
    const std::vector<UtilityNS::DetectedObject>& obj_list)
{
    ...
    double horizontal_safety_distance = car_info.width/2 + params.horizontalSafetyDistancel;

    std::vector<double> horizomtal_obs_distance(rollouts.size(), DBL_MAX);
    std::vector<double> vertical_obs_distance(rollouts.size(), DBL_MAX);
    // for (int i = 0; i < rollouts.size(); i++) {
    //     for (int j = 0; j < allContourPoints.size(); j++) {
    //         UtilityNS::WayPoint p = allContourPoints[j];
    //         double vertical_distance = 0;
    //         for (int k = 0; k < rollouts[i].size(); k++) {
    //             double horizontal_distance = std::sqrt(std::pow(p.pos.x - rollouts[i][k].pos.x,2) + std::pow(p.pos.y - rollouts[i][k].pos.y, 2));
    //             if (k > 0) {
    //                 vertical_distance += std::sqrt(std::pow(rollouts[i][k].pos.x - rollouts[i][k-1].pos.x,2) + std::pow(rollouts[i][k].pos.y - rollouts[i][k-1].pos.y, 2));
    //             }

    //             horizomtal_obs_distance[i] = std::min(horizomtal_obs_distance[i], horizontal_distance);
                
    //             if (horizontal_distance < horizontal_safety_distance) {
    //                 vertical_obs_distance[i] = std::min(vertical_obs_distance[i], vertical_distance);
    //             }
    //         }
    //     }
    // }
    for (int i = 0; i < rollouts.size(); i++) {
        ...
        for (int j = 0; j < rollouts[i].size(); j++) {
            // printf("current_pose: %.2f, %.2f, rollouts[%d][%d].pose: %.2f, %.2f\n", current_pose.pos.x, current_pose.pos.y, i, j, rollouts[i][j].pos.x, rollouts[i][j].pos.y);
            for(const auto& p: allContourPoints) {
                hori_distance = std::min(hori_distance, std::sqrt(std::pow(p.pos.x - rollouts[i][j].pos.x, 2) + std::pow(p.pos.y - rollouts[i][j].pos.y, 2)));
                // if (hori_distance < horizontal_safety_distance)
                //     printf("P: %2.f, %.2f, ->distance=%.2f\n", p.pos.x, p.pos.y, std::min(hori_distance, std::sqrt(std::pow(p.pos.x - rollouts[i][j].pos.x, 2) + std::pow(p.pos.y - rollouts[i][j].pos.y, 2))));
            ...
            if (j > 0) {
                vert_distance += std::sqrt(std::pow(rollouts[i][j].pos.x - rollouts[i][j-1].pos.x,2) + std::pow(rollouts[i][j].pos.y - rollouts[i][j-1].pos.y, 2));
            }
        ...
        horizomtal_obs_distance[i] = hori_distance;
        vertical_obs_distance[i] = vert_distance;
    }

    // printf("hori:");
    // for (const double& d: horizomtal_obs_distance) {
    //     printf(" %.2f", d);
    // }
    // printf("\n");

    // printf("vert:");
    // for (const double& d: vertical_obs_distance) {
    //     printf(" %.2f", d);
    // }
    // printf("\n");

    // if存在多条没有障碍物的路径, 选择距离中心线最近的一条
    int mid = plan_params.rollOutNumber / 2;
    int over_distance = 1000;
    int best_index = -1;
    for (int i = 0; i < horizomtal_obs_distance.size(); i++) {
        if (horizomtal_obs_distance[i] > horizontal_safety_distance) {
            int d = std::abs(i-mid) < over_distance;
            if (best_index == -1) {
                over_distance = d;
                best_index = i;
                continue;
    ...
    if (best_index != -1) {
        best_trajectory.bBlocked = false;
        best_trajectory.index = best_index;
        return best_trajectory;
    }

    // 选距离mid最近且vertical 障碍物最远的一条
    over_distance = 1000;
    best_index = -1;
    for (int i = 0; i < vertical_obs_distance.size(); i++) {
        if (horizomtal_obs_distance[i] > car_info.width/2+params.horizontalSafetyDistancel/2 && vertical_obs_distance[i] > vertical_obs_distance[currIndex]+0.1) {
            int d = std::abs(i - mid);
            if (best_index == -1) {
                over_distance = d;
                best_index = i;
    ...
    if (best_index != -1) {
        ...
        best_trajectory.bBlocked = true;
        best_index = currIndex;
    }
    pre_best_index = best_index;

    // ROS_INFO("Current path index: %d target path index: %d, target_path_obs_horizontal_distance: %.2f (safety > %.2f), target_path_obs_vertical_distance: %.2f", currIndex, best_index, horizomtal_obs_distance[best_index], horizontal_safety_distance, vertical_obs_distance[best_index]);
    ...
    const UtilityNS::CAR_BASIC_INFO& carInfo)
{
    ...
    msg_polygon.header.frame_id = "map";
    geometry_msgs::Point32 p_bottom_left;
    p_bottom_left.x = bottom_left.x;
    p_bottom_left.y = bottom_left.y;
    p_bottom_left.z = bottom_left.z;
    msg_polygon.polygon.points.push_back(p_bottom_left);
    geometry_msgs::Point32 p_bottom_right;
    p_bottom_right.x = bottom_right.x;
    p_bottom_right.y = bottom_right.y;
    p_bottom_right.z = bottom_right.z;
    msg_polygon.polygon.points.push_back(p_bottom_right);
    geometry_msgs::Point32 p_top_right;
    p_top_right.x = top_right.x;
    p_top_right.y = top_right.y;
    p_top_right.z = top_right.z;
    msg_polygon.polygon.points.push_back(p_top_right);
    geometry_msgs::Point32 p_top_left;
    p_top_left.x = top_left.x;
    p_top_left.y = top_left.y;
    p_top_left.z = top_left.z;
    msg_polygon.polygon.points.push_back(p_top_left);
    pub_critical_safe_area.publish(msg_polygon);

    // double lateralDist;  // 横向
