...
#include <automotive_msgs/CmdVehicleControl.h>
...
uint8_t _auto_pilot_status = automotive_msgs::SmartcarSolution::Response::SMARTCAR_LIDAR;
...
//     const autoware_config_msgs::ConfigNDT::ConstPtr& input) {
...
//             std::cout << "initial_pose.roll: " << initial_pose.roll
//                       << std::endl;
//             std::cout << "initial_pose.pitch: " << initial_pose.pitch
//                       << std::endl;
...
static void task_control_callback( const automotive_msgs::CmdVehicleControlConstPtr& input ) {
    if ( input->cmd == input->CMD_RESET_SUBWAY ) {
        ROS_INFO("[ndt_localization] Received command reset subway.");
        reset();
    }
}
...
static void map_callback(const sensor_msgs::PointCloud2::ConstPtr& input) {
    ...
    if (true) {
        ...
        if (_method_type == MethodType::PCL_GENERIC) {
            ...
            std::shared_ptr<gpu::GNormalDistributionsTransform>
                new_anh_gpu_ndt_ptr = std::make_shared<gpu::GNormalDistributionsTransform>();
...
bool _cb_service_function_switch(automotive_msgs::FunctionSwitch::Request &req, automotive_msgs::FunctionSwitch::Response &res) {
    if ( req.switch_to != _FUNCTION_STATUS ) {
        switch (req.switch_to)
        {
        case automotive_msgs::FunctionSwitchResponse::FUNCTION_ENABLE:
            _FUNCTION_STATUS = req.switch_to;
            _enable_sub_init_pose = true;
            ROS_INFO("[ndt localization] Function ON - currently only for subscribe initialpose");
            break;

        case automotive_msgs::FunctionSwitchResponse::FUNCTION_DISABLE:
            _FUNCTION_STATUS = req.switch_to;
            _enable_sub_init_pose = false;
            ROS_INFO("[ndt localization] Function OFF - currently only for subscribe initialpose");
            break;
...
bool _cb_service_auto_pilot(automotive_msgs::SmartcarSolution::Request &req, automotive_msgs::SmartcarSolution::Response &res) {
    if ( req.is_set && _auto_pilot_status != req.set_solution ) {
        if (req.set_solution == automotive_msgs::SmartcarSolution::Response::SMARTCAR_LIDAR){
            _auto_pilot_status = req.set_solution;
            ROS_INFO("[ndt_localization] Switch to auto pilot mode: Lidar");
        }else if (req.set_solution == automotive_msgs::SmartcarSolution::Response::SMARTCAR_RTKGPS){
            _auto_pilot_status = req.set_solution;
            ROS_INFO("[ndt_localization] Switch to auto pilot mode: RTK-GPS");
        }else
            ROS_ERROR("[ndt_localization] Invalid auto pilot mode: %d", req.set_solution);
    }

    res.current_solution = _auto_pilot_status;
    return true;
}
...
int main(int argc, char** argv) {
    ...
    if (_output_log_data) {
        ...
        std::string directory_name = "/tmp/Automotive/log/ndt_matching";
    ...
    //     } else {
    //         ROS_WARN(
    //             "Query base_link to primary lidar frame through "
    ...
    ros::Subscriber task_control_sub = 
        nh.subscribe("/gundam/task_control", 5, task_control_callback);

    // pthread_t thread;
    // pthread_create(&thread, NULL, thread_func, NULL);

    ros::ServiceServer function_service = nh.advertiseService("/function_switch/ndt_localization", _cb_service_function_switch);	
    ros::ServiceServer service_auto_pilot_manager = nh.advertiseService("/ndt_localization/service_solution_status", _cb_service_auto_pilot);
