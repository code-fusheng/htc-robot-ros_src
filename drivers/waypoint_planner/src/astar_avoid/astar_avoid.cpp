  ...
  , terminate_thread_(false)
{
  ...
  private_nh_.param<std::string>("current_pose_topic", current_pose_topic, "current_pose topic not set");

  // safety_waypoints_pub_ = nh_.advertise<autoware_msgs::Lane>("safety_waypoints", 1, true);
  smartcar_safety_waypoints_pub_ = nh_.advertise<nav_msgs::Path>("safety_waypoints", 1, true);
  costmap_sub_ = nh_.subscribe("costmap", 1, &AstarAvoid::costmapCallback, this);
  current_pose_sub_ = nh_.subscribe(current_pose_topic, 1, &AstarAvoid::currentPoseCallback, this);
  // current_velocity_sub_ = nh_.subscribe("current_velocity", 1, &AstarAvoid::currentVelocityCallback, this);
  vehicle_status_sub_ = nh_.subscribe("/vehicle_status", 1, &AstarAvoid::vehicleStatusCallback, this);
  // base_waypoints_sub_ = nh_.subscribe("base_waypoints", 1, &AstarAvoid::baseWaypointsCallback, this);
  smartcar_base_waypoints_sub = nh_.subscribe("/global_path", 1, &AstarAvoid::smartcarBaseWaypointsCallback, this);
...
void AstarAvoid::vehicleStatusCallback(const can_msgs::vehicle_status& msg) {
  geometry_msgs::TwistStamped twist_msg;
  twist_msg.twist.linear.x = msg.cur_speed;
  current_velocity_initialized_ = true;
}

void AstarAvoid::smartcarBaseWaypointsCallback(const smartcar_msgs::Lane& msg) {
  autoware_msgs::Lane new_msg;
  for (const auto p: msg.waypoints) {
    autoware_msgs::Waypoint w;
    w.pose = p.pose;
    new_msg.waypoints.push_back(w);
  }
  this->baseWaypointsCallback(new_msg);
  std_msgs::Int32 t;
  t.data = -1;
  this->closestWaypointCallback(t);
...
bool AstarAvoid::checkInitialized()
{
  ...
  // if ( !current_pose_initialized_ ) ROS_WARN("[AStar] current pose uninitialized");
  // if ( !closest_waypoint_initialized_ ) ROS_WARN("[AStar] closest_waypoint_initialized_ is false");
  // if ( !base_waypoints_initialized_ ) ROS_WARN("[AStar] base_waypoints_initialized_ is false");
  // if ( closest_waypoint_index_ < 0 ) ROS_WARN("[AStar] closest_waypoint_index_ = %d", closest_waypoint_index_);
...
void AstarAvoid::publishWaypoints()
{
  ...
  while (!terminate_thread_)
  {
    ...
    if (safety_waypoints.waypoints.size() > 0)
    {
      ...
      nav_msgs::Path path;
      for (const auto p: safety_waypoints.waypoints) {
        path.poses.push_back(p.pose);
      }
      path.header.frame_id = "map";
      smartcar_safety_waypoints_pub_.publish(path);
    }
