...
namespace GLOBAL_PLANNER
{
    void global_plan::init()
    {
        ...
        nh.param<std::string>("data_base_folder", data_base, "");
        if (data_base == "")
        {
        ...
        path = data_base + "planning/default_path/";
        ...
        sub_startPose = nh.subscribe("/current_pose", 1, &GLOBAL_PLANNER::global_plan::update_current_pose, this);
    ...
    void global_plan::endPose_cb(const geometry_msgs::PoseStampedConstPtr &msg)
    {
        ...
        if (util::distance2points(current_pose.pose.position, msg->pose.position) < 15)
        {
            std::cout << "start: " << current_pose.pose.position.x << " " << current_pose.pose.position.y << std::endl;
            std::cout << "end: " << end_pose.pose.position.x << " " << end_pose.pose.position.y << std::endl;
            std::cout << "dist: " << util::distance2points(start_pose.pose.position, msg->pose.position) << std::endl;
